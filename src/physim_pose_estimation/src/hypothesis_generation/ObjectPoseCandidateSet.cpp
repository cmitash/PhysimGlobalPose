...
void getProbableTransformsSuper4PCS(std::string input1, std::string input2, std::string input3, std::pair<Eigen::Isometry3d, float> &bestHypothesis, 
            ...
            std::map<std::vector<int>, int> PPFMap, int max_count_ppf, Eigen::Matrix3f camIntrinsic, std::string objName, std::string scenePath);
...
namespace pose_candidates{
	...
	void ObjectPoseCandidateSet::generate(std::string objName, std::string scenePath, 
				...
				std::map<std::vector<int>, int> PPFMap, int max_count_ppf, Eigen::Matrix4f camPose, Eigen::Matrix3f camIntrinsic/*, std::thread &th_id*/){
